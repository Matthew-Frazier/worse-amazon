...
class Reviews extends React.Component {
  ...
  componentDidMount() {
    // this id belongs to the item itself
    const { id } = this.props
        ...
        <Card.Content>
          ...
          <div style={{ display: 'flex', alignSelf: 'flex-end', marginTop: '10px', width: '100px' }}>
            ...
            <Link to={{
              ...
              // this state will be found in location props, so when it is picked up in the componentDidMount() for the reviews form it can be used for the edit review if statement
              state: {
                item_id: this.props.id,
              }
            }}
